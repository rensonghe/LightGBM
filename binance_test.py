...
cols_list = ['closetime', 'ask_price1', 'ask_size1', 'bid_price1', 'bid_size1',
             ...
             'center_deri_two', 'quasi', 'last_range',
...
def test_data_consistency(test_data: float, col_name: str, ori_data=np.array([]), index=0, sum_num_=0,
                          df_2d_=np.atleast_2d(np.zeros(85)), col_dict={}):
    if test_data == 0:
...
def factor_calculation(df: bytearray, index, feat_dict: dict, sum_num_: int, df_2d_, col_dict):
    ...
    # for i in range(1, 6):
    ask = df[col_dict['ask_price1']] * df[col_dict['ask_size1']] + df[col_dict['ask_price2']] * df[
        col_dict['ask_size2']] + \
          df[col_dict['ask_price3']] * df[col_dict['ask_size3']] + df[col_dict['ask_price4']] * df[
              col_dict['ask_size4']] + \
          df[col_dict['ask_price5']] * df[col_dict['ask_size5']] + df[col_dict['ask_price6']] * df[
              col_dict['ask_size6']] + \
          df[col_dict['ask_price7']] * df[col_dict['ask_size7']] + df[col_dict['ask_price8']] * df[
              col_dict['ask_size8']] + \
          df[col_dict['ask_price9']] * df[col_dict['ask_size9']] + df[col_dict['ask_price10']] * df[
              col_dict['ask_size10']]
    bid = df[col_dict['bid_price1']] * df[col_dict['bid_size1']] + df[col_dict['bid_price2']] * df[
        col_dict['bid_size2']] + \
          df[col_dict['bid_price3']] * df[col_dict['bid_size3']] + df[col_dict['bid_price4']] * df[
              col_dict['bid_size4']] + \
          df[col_dict['bid_price5']] * df[col_dict['bid_size5']] + df[col_dict['bid_price6']] * df[
              col_dict['bid_size6']] + \
          df[col_dict['bid_price7']] * df[col_dict['bid_size7']] + df[col_dict['bid_price8']] * df[
              col_dict['bid_size8']] + \
          df[col_dict['bid_price9']] * df[col_dict['bid_size9']] + df[col_dict['bid_price10']] * df[
              col_dict['bid_size10']]
    ask_v = df[col_dict['ask_size1']] + df[col_dict['ask_size2']] + df[col_dict['ask_size3']] + df[
        col_dict['ask_size4']] + df[col_dict['ask_size5']] + \
            df[col_dict['ask_size6']] + df[col_dict['ask_size7']] + df[col_dict['ask_size8']] + df[
                col_dict['ask_size9']] + df[col_dict['ask_size10']]
    bid_v = df[col_dict['bid_size1']] + df[col_dict['bid_size2']] + df[col_dict['bid_size3']] + df[
        col_dict['bid_size4']] + df[col_dict['bid_size5']] + \
            df[col_dict['bid_size6']] + df[col_dict['bid_size7']] + df[col_dict['bid_size8']] + df[
                col_dict['bid_size9']] + df[col_dict['bid_size10']]
    ask /= ask_v
    bid /= bid_v
    ...
    if len(feat_dict['size']) > 10:
        bvs = feat_dict['bid_size1'][-1] + feat_dict['bid_size2'][-1] + feat_dict['bid_size3'][-1] + \
              ...
              feat_dict['bid_size6'][-1] + feat_dict['bid_size7'][-1] + feat_dict['bid_size8'][-1] + \
              feat_dict['bid_size9'][-1] + feat_dict['bid_size10'][-1]
    ...
    if len(feat_dict['size']) > 10:
        avs = feat_dict['ask_size1'][-1] + feat_dict['ask_size2'][-1] + feat_dict['ask_size3'][-1] + \
              ...
              feat_dict['ask_size6'][-1] + feat_dict['ask_size7'][-1] + feat_dict['ask_size8'][-1] + \
              feat_dict['ask_size9'][-1] + feat_dict['ask_size10'][-1]
    ...
    # for i in range(1, 6):
    avs_aps = df[col_dict['ask_price1']] * df[col_dict['ask_size1']] + df[col_dict['ask_price2']] * df[
        col_dict['ask_size2']] + \
              df[col_dict['ask_price3']] * df[col_dict['ask_size3']] + df[col_dict['ask_price4']] * df[
                  col_dict['ask_size4']] + \
              df[col_dict['ask_price5']] * df[col_dict['ask_size5']] + df[col_dict['ask_price6']] * df[
              ...
              df[col_dict['ask_price9']] * df[col_dict['ask_size9']] + df[col_dict['ask_price10']] * df[
                  col_dict['ask_size10']]
    bvs_bps = df[col_dict['bid_price1']] * df[col_dict['bid_size1']] + df[col_dict['bid_price2']] * df[
        col_dict['bid_size2']] + \
              df[col_dict['bid_price3']] * df[col_dict['bid_size3']] + df[col_dict['bid_price4']] * df[
                  col_dict['bid_size4']] + \
              df[col_dict['bid_price5']] * df[col_dict['bid_size5']] + df[col_dict['bid_price6']] * df[
              ...
              df[col_dict['bid_price9']] * df[col_dict['bid_size9']] + df[col_dict['bid_price10']] * df[
                  col_dict['bid_size10']]
    avs = df[col_dict['ask_size1']] + df[col_dict['ask_size2']] + df[col_dict['ask_size3']] + df[
        col_dict['ask_size4']] + df[col_dict['ask_size5']] + \
          df[col_dict['ask_size6']] + df[col_dict['ask_size7']] + df[col_dict['ask_size8']] + df[
              col_dict['ask_size9']] + df[col_dict['ask_size10']]
    bvs = df[col_dict['bid_size1']] + df[col_dict['bid_size2']] + df[col_dict['bid_size3']] + df[
        col_dict['bid_size4']] + df[col_dict['bid_size5']] + \
          df[col_dict['bid_size6']] + df[col_dict['bid_size7']] + df[col_dict['bid_size8']] + df[
              col_dict['bid_size9']] + df[col_dict['bid_size10']]
    ...
    if len(feat_dict['ask_price1']) > 1:
        ask_ob_values_last = np.hstack((feat_dict['ask_price1'][-2], feat_dict['ask_size1'][-2],
                                        ...
                                        feat_dict['ask_price6'][-2], feat_dict['ask_size6'][-2],
                                        feat_dict['bid_price6'][-2], feat_dict['bid_size6'][-2],
                                        feat_dict['ask_price7'][-2], feat_dict['ask_size7'][-2],
                                        feat_dict['bid_price7'][-2], feat_dict['bid_size7'][-2],
                                        feat_dict['ask_price8'][-2], feat_dict['ask_size8'][-2],
                                        feat_dict['bid_price8'][-2], feat_dict['bid_size8'][-2],
                                        feat_dict['ask_price9'][-2], feat_dict['ask_size9'][-2],
                                        feat_dict['bid_price9'][-2], feat_dict['bid_size9'][-2],
                                        feat_dict['ask_price10'][-2], feat_dict['ask_size10'][-2],
                                        feat_dict['bid_price10'][-2], feat_dict['bid_size10'][-2]))
        ask_ob_values_now = np.hstack((feat_dict['ask_price1'][-1], feat_dict['ask_size1'][-1],
                                       ...
                                       feat_dict['ask_price6'][-1], feat_dict['ask_size6'][-1],
                                       feat_dict['bid_price6'][-1], feat_dict['bid_size6'][-1],
                                       feat_dict['ask_price7'][-1], feat_dict['ask_size7'][-1],
                                       feat_dict['bid_price7'][-1], feat_dict['bid_size7'][-1],
                                       feat_dict['ask_price8'][-1], feat_dict['ask_size8'][-1],
                                       feat_dict['bid_price8'][-1], feat_dict['bid_size8'][-1],
                                       feat_dict['ask_price9'][-1], feat_dict['ask_size9'][-1],
                                       feat_dict['bid_price9'][-1], feat_dict['bid_size9'][-1],
                                       feat_dict['ask_price10'][-1], feat_dict['ask_size10'][-1],
                                       feat_dict['bid_price10'][-1], feat_dict['bid_size10'][-1]))
    ...
    wb = df[col_dict['bid_size1']] * w[0] + df[col_dict['bid_size2']] * w[1] + df[col_dict['bid_size3']] * w[2] + df[
        col_dict['bid_size4']] * w[3] + df[col_dict['bid_size5']] * w[4] + \
         df[col_dict['bid_size6']] * w[5] + df[col_dict['bid_size7']] * w[6] + df[col_dict['bid_size8']] * w[7] + df[
             col_dict['bid_size9']] * w[8] + df[col_dict['bid_size10']] * w[9]
    wa = df[col_dict['ask_size1']] * w[0] + df[col_dict['ask_size2']] * w[1] + df[col_dict['ask_size3']] * w[2] + df[
        col_dict['ask_size4']] * w[3] + df[col_dict['ask_size5']] * w[4] + \
         df[col_dict['ask_size6']] * w[5] + df[col_dict['ask_size7']] * w[6] + df[col_dict['ask_size8']] * w[7] + df[
             col_dict['ask_size9']] * w[8] + df[col_dict['ask_size10']] * w[9]
...
if __name__ == "__main__":
    ...
    data_agg = get_data_from_minio('binance_swap_u', symbol_list, 'datafile/feat/songhe/', index_name='closetime',
                                   start_time=start_time, end_time=end_time)
    ...
    t1 = time.time()
    sum_num = 0
    df_2d = np.atleast_2d(np.zeros(85))
    for i in range(1000):
        sum_num = 0
        # test_columns = []
        # print(i)
