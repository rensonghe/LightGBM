...
for month in tqdm(range(9, 10)):
    ...
    depth_base = pq.ParquetDataset('datafile/tick/order_book_100ms/gate_swap_u', filters=filters, filesystem=minio)
...
def bp_rank(depth, trade, rolling=100):
    # print(depth.bid_price1.rolling(rolling).rank())
...
def buy_increasing(depth, trade, rolling=100):
    v = trade['size'].copy()
    v[v < 0] = 0
    print((v.fillna(0)).rolling(2 * rolling).sum() + 1)
...
def avg_turnover(depth, trade, rolling=500):
    return depth[
        ['ask_size1', 'ask_size2', 'ask_size3', 'ask_size4', "ask_size5", 'bid_size1', 'bid_size2', 'bid_size3',
         'bid_size4', "bid_size5"]].sum(axis=1)
...
def positive_ratio(depth, trade,rolling=20 * 3):
    ...
    buy_positive['turnover'] = np.where(trade['price']>=depth['ask_price1'].shift(1), buy_positive['turnover'], 0)
...
# def intraday_moving_average(trade,windows = 60):
#     return trade['price'].fillna(0).groupby(trade['price'].index.date).apply(lambda x:x.rolling(windows).mean().shift())

# def trend_to_ma(trade , windows = 60,dir = "over",return_ma=False):
#     ma = intraday_moving_average(trade, windows)
#     if dir == "over":
#         _ = trade['price'][trade['price']>ma].index
#     else:
#         _ = trade['price'][trade['price']<ma].index
...
def add_factor_process(depth, trade):
    ...
    df['depth_price_range'] = depth_price_range(depth=depth, trade=None,rolling=100)
    df['bp_rank'] = bp_rank(depth=depth, trade=None, rolling=100)
    # df['bp_rank_300'] = bp_rank(depth=depth, trade=None, rolling=300)
    df['ap_rank'] = ap_rank(depth=depth, trade=None, rolling=100)
    # df['ap_rank_300'] = ap_rank(depth=depth, trade=None, rolling=300)
...
#%%
add_factor['wap1'] = calc_wap1(add_factor)
#%%
import os
import pyarrow.parquet as pq
import pyarrow
