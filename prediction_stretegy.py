    ...
    def execute(self):
        ...
        for i in self.bt_data.values.tolist():
            ...
            if i[self.column_list.index('predict')] > 0:
                ...
                #     print(self.depth)
                self._place_order(create_timestamp=closetime, symbol=self.symbol, price=bid_price1*1.00001, side='buy',
                                  size=buy_size, order_type='gtc', status='open')
                self.place_buy_order = bid_price1 *1.00001
                self._place_order(create_timestamp=closetime, symbol=self.symbol, price=ask_price1*0.99999, side='sell',
                                  size=sell_size, order_type='gtc', status='open')
                self.place_sell_order = ask_price1*0.99999
                # print(self.depth)
